%% Planner
initialLocation = [1,1; 8,1; 1,8; 8,8]; %representing the initial location of each robot
goalLocation = [3,1; 5,2; 6,3; 2,2];% teh position of tasks
M = zeros(4,4);
for i = 1:4
    for j = 1:4
...
for i = 1:4
    for j = 1:4
        if A(i,j) == 1
            ...
            text(initialLocation(i,1), initialLocation(i,2),OrderOfRobot,'FontSize',10)
            text(initialLocation(i,1), initialLocation(i,2),'o','color','g')
            text(goalLocation(j,1), goalLocation(j,2),OrderOfGoal,'FontSize',12)
