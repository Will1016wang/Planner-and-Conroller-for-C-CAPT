...
for i = 1:4
    for j = 1:4
        M(i,j) = norm((initialLocation(i,:) - goalLocation(j,:)).^2);
    end
end
[A,C]= munkres(M); % A is the optimal assignment matrix, C is the total minimum cost
pi = A.*M; % calculate pi*, In the CAPT paper, pi is represented by A and D is represented by M.
...
% in the paper, it seems I need to set up a maximum valocity and define the trajectory by a polynomial function. However, i need to talk to Walker to do this part afterwards.
